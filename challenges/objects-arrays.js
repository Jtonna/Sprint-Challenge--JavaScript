...
const contactInfo = [];
for(i = 0; i < graduates.length; i++){
  studentName = graduates[i]['first_name'];
  studentEmail = graduates[i]['email'];
  combineInfo = studentName + ' ' + studentEmail;
  contactInfo.push(combineInfo);
};
contactInfo.sort();
...
const uni = [];
for( i = 0; i < graduates.length; i++){
  containsUni = graduates[i]['university'];
  if (containsUni.includes('Univ')){
...
*/
// const reducer = (accumilator, currentValue) => accumilator + currentValue;
// console.log(zooAnimals.reduce(reducer))

let populationTotal = zooAnimals.reduce(function(x,y){
